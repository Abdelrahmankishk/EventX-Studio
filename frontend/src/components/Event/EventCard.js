...
const EventCard = ({ event, isAdmin, onEventDeleted, fetchEvents }) => {
  ...
  const handleDeleteEvent = async () => {
    ...
    if (window.confirm("Are you sure you want to delete this event?")) {
      try {
        await axios.delete(
          `https://eventx-studio-production-ffea.up.railway.app/events/${event._id}`,
          {
  ...
  const handleBookEvent = async () => {
    ...
    try {
      const response = await axios.post(
        `https://eventx-studio-production-ffea.up.railway.app/events/${event._id}/book`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const getUserName = async (userId) => {
    try {
      const response = await axios.get(
        `https://eventx-studio-production-ffea.up.railway.app/user/${userId}`
      );
    ...
    const fetchBookedUsers = async () => {
      if (showBookings && event.bookedBy?.length > 0) {
        const users = await Promise.all(
          event.bookedBy.map((userId) => getUserName(userId))
        );
                ...
                {isAlreadyBooked ? (
                  ...
                  <motion.button
                    className={`action-button ${
                      isEventFull ? "disabled-action" : "primary-action"
                    }`}
