  ...
  const fetchBookedEvents = async () => {
    try {
      ...
      const response = await axios.get(
        "https://eventx-studio-production-ffea.up.railway.app/user/booked-events",
        {
          headers: {
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.message || "Failed to load booked events"
        );
      }
    } catch (error) {
      console.error("Error fetching booked events:", error);
      const errorMessage =
        error.response?.data?.message || "Failed to load your booked events";
  ...
  const handleCancelBooking = async (eventId) => {
    try {
      const response = await axios.delete(
        `https://eventx-studio-production-ffea.up.railway.app/events/${eventId}/book`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
          ...
          <div className="events-grid">
            {[1, 2, 3].map((n) => (
              <div key={n} className="event-card">
                <div
                  className="event-image loading-skeleton"
                  style={{ height: "200px" }}
                ></div>
        ...
        >
          {events.length > 0 ? (
            events.map((event) => (
              ...
              >
                <img
                  src={event.image || "/assets/live.png"}
                  alt={event.title}
